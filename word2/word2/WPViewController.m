...
- (void)viewDidLoad
{
    ...
    self.title = @"编辑器";
    self.delegate = self;
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"预览/编辑"
...
{
    KLog(@"Editor title did change: %@", self.titleText);
}

- (void)editorTextDidChange:(WPEditorViewController *)editorController
{
    KLog(@"Editor body text changed: %@", self.bodyText);
}

- (void)editorViewController:(WPEditorViewController *)editorViewController fieldCreated:(WPEditorField*)field
{
    KLog(@"Editor field created: %@", field.nodeId);
}
...
- (void)editorViewController:(WPEditorViewController *)editorViewController mediaRemoved:(NSString *)mediaID
{
    ...
    [progress cancel];
    KLog(@"Media Removed: %@", mediaID);
}

- (void)editorFormatBarStatusChanged:(WPEditorViewController *)editorController
                             enabled:(BOOL)isEnabled
{
    KLog(@"Editor format bar status is now %@.", (isEnabled ? @"enabled" : @"disabled"));
}
